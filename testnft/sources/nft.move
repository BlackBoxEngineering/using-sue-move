...
module testnft::nft {
  ...
  */
  struct NonFungibleToken has key, store {
    id: UID,
    name: String,
    description: String,
    image: Url,
  ...
  */
  public fun mint_nft(
    ...
    if (payment_amount > 1) {
      ...
      change = coin::split(payment_coin, change_amount, ctx);
    } else {
      change = coin::zero(ctx);
    };
    ...
    balance::join(&mut minter_cap.sales, coin::into_balance(payment_coin);
  ...
  */
  public fun url(nft: &NonFungibleToken): Url {
    ...
    // Helpers
    public fun delete_nft(nft: NonFungibleToken) {
